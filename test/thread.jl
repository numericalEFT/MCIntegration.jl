@testset "Threads" begin
    ...
    Threads.@threads for i in 1:3
        ...
        result = integrate((X, f, c) -> f[1] = (X[1])^i; var=(Continuous(0.0, 1.0),), dof=[[1,],], print=-1, solver=:vegas)
        println(i, " -> ", result.mean, "+-", result.stdev)
        check(result, 1 / (1 + i))
        result = integrate((X, f, c) -> f[1] = (X[1])^i; var=(Continuous(0.0, 1.0),), dof=[[1,],], print=-1, solver=:vegasmc)
        check(result, 1 / (1 + i))
